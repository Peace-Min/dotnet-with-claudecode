...
// records it to ~/.wpf-net472-airgap-dev-pack-mcp/config.json so WpfDevPackMcp can read it.
...
if (string.IsNullOrWhiteSpace(path))
{
    ...
    Console.Error.WriteLine("Usage: /wpf-net472-airgap-dev-pack:set-repo-path <path-to-local-clone> [branch]");
    return 1;
}
...
else
{
    ...
    if (!Directory.Exists(Path.Combine(full, "wpf-net472-airgap-dev-pack")))
    {
        warnings.Add("No 'wpf-net472-airgap-dev-pack' folder found at the path — confirm this is the dotnet-with-claudecode repo root.");
    }
}

var baseDir = Path.Combine(
    Environment.GetFolderPath(Environment.SpecialFolder.UserProfile), ".wpf-net472-airgap-dev-pack-mcp");
