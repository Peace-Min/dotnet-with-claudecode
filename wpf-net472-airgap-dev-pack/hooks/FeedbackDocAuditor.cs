...
// Audits wpf-net472-airgap-dev-pack feedback markdown files for identifying information
// after Write/Edit operations.
//
// Wired through:
//   skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md frontmatter `hooks:`
// NOT through the plugin-global hooks/hooks.json. As a skill-scoped hook,
// it fires only while the `collecting-wpf-net472-airgap-dev-pack-feedback` skill is
// active, so it will not run during unrelated edits to feedback markdown
// files in other sessions.
//
// Matches: files whose name ends with "-wpf-net472-airgap-dev-pack-feedback.md"
...
// collecting-wpf-net472-airgap-dev-pack-feedback skill.
//
...
if (!fileName.EndsWith("-wpf-net472-airgap-dev-pack-feedback.md", StringComparison.OrdinalIgnoreCase))
    return;
...
Console.Error.WriteLine($"[wpf-net472-airgap-dev-pack] Feedback document audit failed: {fileName}");
...
Console.Error.WriteLine();
Console.Error.WriteLine("Re-run Step 5 (Audit) of `collecting-wpf-net472-airgap-dev-pack-feedback`:");
...
internal static partial class FeedbackDocAuditor
{
    public static List<AuditViolation> Audit(string content)
    {
        ...
        if (!RequiredH1().IsMatch(content))
        {
            violations.Add(new AuditViolation(
                "F-006",
                "Missing required H1 header (`# wpf-net472-airgap-dev-pack Feedback - <title>` or `# wpf-net472-airgap-dev-pack 피드백 - <title>`).",
                string.Empty));
        }
    ...
    [GeneratedRegex(@"^#\s+wpf-net472-airgap-dev-pack\s+(?:Feedback|피드백)",
        RegexOptions.Multiline | RegexOptions.IgnoreCase)]
