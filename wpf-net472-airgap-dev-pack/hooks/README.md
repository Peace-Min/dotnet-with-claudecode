...
| **FeedbackDocAuditor** | PostToolUse (skill-scoped) | Audits `*-wpf-net472-airgap-dev-pack-feedback.md` files for identifying information; exits with code 2 on violation. Wired through `skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md` frontmatter (not `hooks.json`), so it fires only while that skill is active. |
