---
description: "Sets the server-managed 'Managed' flag in ~/.wpf-net472-airgap-dev-pack-mcp/state.json. Managed=true lets WpfDevPackMcp git reset --hard its clone to origin/<branch> on refresh (only safe for a dedicated clone the server owns); Managed=false makes refresh non-destructive (ff-only when clean, skip when dirty) — required when repoPath is your active working repo. Use to stop the MCP from resetting your working tree, or to hand a dedicated clone back to server management."
...
Sets `Managed` in `~/.wpf-net472-airgap-dev-pack-mcp/state.json` (preserving `LastPullUtc`).
...
`/wpf-net472-airgap-dev-pack:show-wpf-net472-airgap-dev-pack-config`.
