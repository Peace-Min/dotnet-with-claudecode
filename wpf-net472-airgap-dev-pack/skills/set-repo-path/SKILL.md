---
description: "Sets the local clone path of the christian289/dotnet-with-claudecode repository so the WpfDevPackMcp server can read WPF knowledge topics from disk. Use when the WpfDevPackMcp server reports the repo path is not configured, when first setting up wpf-net472-airgap-dev-pack on a machine, or when moving the local clone. Writes ~/.wpf-net472-airgap-dev-pack-mcp/config.json."
...
- `/wpf-net472-airgap-dev-pack:show-wpf-net472-airgap-dev-pack-config` — view the current config.json /
  state.json paths and values (and any `WPFDEVPACK_REPO_PATH` env override).
- `/wpf-net472-airgap-dev-pack:set-repo-branch <branch>` — change the tracked branch
  (preserves repoPath).
- `/wpf-net472-airgap-dev-pack:set-repo-managed <true|false>` — control whether refresh
