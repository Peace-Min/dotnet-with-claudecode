---
description: "Generates a role-differentiated chat bubble DataTemplate (color, alignment, asymmetric margin, content-sized) plus the turn ViewModel for an ItemsControl of chat turns. Use when laying out a chat/conversation list where user and assistant turns must look distinct. Usage: /wpf-net472-airgap-dev-pack:make-wpf-chat-bubble-template <TurnName>"
...
- The bubble hosts a `MarkdownPresenter` (generate it with
  `/wpf-net472-airgap-dev-pack:make-wpf-markdown-presenter`) so each turn renders selectable
  markdown.
...
```bash
/wpf-net472-airgap-dev-pack:make-wpf-chat-bubble-template ChatTurnViewModel
```
...
> **Skip this file if `/wpf-net472-airgap-dev-pack:make-wpf-chatclient` already emitted a
...
- Sibling scaffolders: `/wpf-net472-airgap-dev-pack:make-wpf-markdown-presenter` (the bubble's
  content control). This template is one of the pieces emitted by the one-button
  `/wpf-net472-airgap-dev-pack:make-wpf-chatclient`.
