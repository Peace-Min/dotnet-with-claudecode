# collecting-wpf-net472-airgap-dev-pack-feedback Skill Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add a human-only wpf-net472-airgap-dev-pack skill that analyzes a WPF working session and emits an accumulable improvement-feedback markdown file, plus migrate one existing feedback-style spec into a new `FeedbackDocs/` folder and sync version/index metadata.

**Architecture:** A new `disable-model-invocation: true` skill under `wpf-net472-airgap-dev-pack/skills/` runs entirely in the foreign WPF session: auto-draft from conversation → interview augmentation via AskUserQuestion → write a timestamped md to the current working directory. No repo-path discovery, no git. One existing spec is `git mv`-ed to repo-root `FeedbackDocs/`. Metadata files (plugin.json, READMEs, Category Index, project Directory Layout) are updated to register the skill and folder.
...
| File | Responsibility |
|------|----------------|
| `wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md` (create) | The skill itself — frontmatter + English workflow checklist + embedded Korean output template |
...
| `wpf-net472-airgap-dev-pack/skills/.claude/CLAUDE.md` (modify) | Register skill in Category Index (new "Meta / Maintenance" row); no Keyword Mapping (no auto-trigger) |
| `wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json` (modify) | Version bump 1.6.3 → 1.6.4 |
| `wpf-net472-airgap-dev-pack/README.md` (modify) | Version badge → 1.6.4 |
| `wpf-net472-airgap-dev-pack/README.ko.md` (modify) | Version badge → 1.6.4 |
...
**Files:**
- Create: `wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md`

- [ ] **Step 1: Write SKILL.md with exact content below**

Create `wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md` containing exactly:

````markdown
---
description: "Analyzes the current WPF working session and produces an accumulable wpf-net472-airgap-dev-pack improvement-feedback markdown file in the current directory. Use when, after doing WPF work with wpf-net472-airgap-dev-pack skills/scaffolders in a project OUTSIDE the dotnet-with-claudecode repo, you want to capture anti-patterns you hand-fixed, missing or outdated skill guidance, mistaken or missed triggers, and scaffolder gaps — without editing wpf-net472-airgap-dev-pack directly. The output md is meant to be contributed to dotnet-with-claudecode/FeedbackDocs via a pull request. User-invocable only."
disable-model-invocation: true
argument-hint: "[topic]"
---

# Collecting wpf-net472-airgap-dev-pack Feedback

This skill turns lessons learned in the current WPF session into a structured
improvement-feedback document for the `wpf-net472-airgap-dev-pack` plugin. It does NOT modify
`wpf-net472-airgap-dev-pack` and does NOT touch git — it only writes one markdown file to the
...
that repo's local clone path differs per machine. Editing `wpf-net472-airgap-dev-pack` from a
...
- wpf-net472-airgap-dev-pack skills that were used, and where their guidance was inaccurate or insufficient
...
```markdown
# wpf-net472-airgap-dev-pack 피드백 — <한 줄 제목>
...
Run:
```
pwsh -NoProfile -Command "$t=Get-Content 'wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md' -Raw; if($t -match '(?m)^model:\s'){Write-Error 'model field present — must be removed'}else{'no model field — ok'}"
```
...
```
git add wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md
git commit -m "feat(wpf-net472-airgap-dev-pack): add collecting-wpf-net472-airgap-dev-pack-feedback skill"
```
...
**Files:**
- Move: `wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-05-18-customcontrol-authoring-upgrade-design.md` → `FeedbackDocs/2026-05-18-customcontrol-authoring-upgrade-design.md`
...
```
git mv wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-05-18-customcontrol-authoring-upgrade-design.md FeedbackDocs/2026-05-18-customcontrol-authoring-upgrade-design.md
```
...
Run:
```
pwsh -NoProfile -Command "Test-Path 'FeedbackDocs/2026-05-18-customcontrol-authoring-upgrade-design.md'; Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-05-18-customcontrol-authoring-upgrade-design.md'; Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-04-03-cmds-pattern-refactoring-design.md'; Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/plans/2026-04-03-cmds-pattern-refactoring.md'"
```
...
`wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-05-18-customcontrol-authoring-upgrade-design.md`
...
```
git add -A
git commit -m "chore(wpf-net472-airgap-dev-pack): migrate customcontrol-authoring feedback spec to FeedbackDocs/"
```
...
**Files:**
- Modify: `wpf-net472-airgap-dev-pack/skills/.claude/CLAUDE.md` (final table `## Skill Category Index`)

- [ ] **Step 1: Add a "Meta / Maintenance" row to the Category Index table**

In `wpf-net472-airgap-dev-pack/skills/.claude/CLAUDE.md`, find the last row of the Category Index table:
...
```
| **Meta / Maintenance** | `collecting-wpf-net472-airgap-dev-pack-feedback` |
```
...
Run:
```
pwsh -NoProfile -Command "$t=Get-Content 'wpf-net472-airgap-dev-pack/skills/.claude/CLAUDE.md' -Raw; if($t -match '\*\*Meta / Maintenance\*\* \| `collecting-wpf-net472-airgap-dev-pack-feedback`'){'category row ok'}else{Write-Error 'category row missing'}; $km=($t -split '## Skill Category Index')[0]; if($km -match 'collecting-wpf-net472-airgap-dev-pack-feedback'){Write-Error 'must NOT appear in Keyword Mapping'}else{'keyword mapping clean'}"
```
...
```
git add wpf-net472-airgap-dev-pack/skills/.claude/CLAUDE.md
git commit -m "docs(wpf-net472-airgap-dev-pack): index collecting-wpf-net472-airgap-dev-pack-feedback skill"
```
...
**Files:**
- Modify: `wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json:4`
- Modify: `wpf-net472-airgap-dev-pack/README.md:9`
- Modify: `wpf-net472-airgap-dev-pack/README.ko.md:9`

- [ ] **Step 1: Bump plugin.json version**

In `wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json`, change line 4 from:
```
  "version": "1.6.3",
```
to:
...
In `wpf-net472-airgap-dev-pack/README.md`, change line 9 from:
```
...
In `wpf-net472-airgap-dev-pack/README.ko.md`, change line 9 from:
```
[![Version](https://img.shields.io/badge/version-1.6.3-blue.svg)](https://github.com/christian289/dotnet-with-claudecode)
```
to:
...
Run:
```
pwsh -NoProfile -Command "$j=Get-Content 'wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json' -Raw | ConvertFrom-Json; Write-Output ('plugin=' + $j.version); $r=Get-Content 'wpf-net472-airgap-dev-pack/README.md' -Raw; $k=Get-Content 'wpf-net472-airgap-dev-pack/README.ko.md' -Raw; if($j.version -eq '1.6.4' -and $r -match 'version-1.6.4-blue' -and $k -match 'version-1.6.4-blue' -and $r -notmatch 'version-1.6.3-blue' -and $k -notmatch 'version-1.6.3-blue'){'version sync ok'}else{Write-Error 'version mismatch'}"
```
...
```
git add wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json wpf-net472-airgap-dev-pack/README.md wpf-net472-airgap-dev-pack/README.ko.md
git commit -m "chore(wpf-net472-airgap-dev-pack): v1.6.4 - register feedback collection skill"
```
...
```
├── wpf-net472-airgap-dev-pack/                 # WPF 전용 플러그인 (현재 유일한 호스팅 대상)
...
```
├── wpf-net472-airgap-dev-pack/                 # WPF 전용 플러그인 (현재 유일한 호스팅 대상)
├── FeedbackDocs/                 # 외부 세션 wpf-net472-airgap-dev-pack 피드백 md 누적 폴더
├── archive-skills/               # microsoft-docs MCP로 대체되어 보관된 구 skill
└── docs/                         # 프로젝트 문서
```
...
Run:
```
pwsh -NoProfile -Command "$t=Get-Content '.claude/CLAUDE.md' -Raw; if($t -match 'FeedbackDocs/\s+# 외부 세션 wpf-net472-airgap-dev-pack 피드백 md 누적 폴더'){'layout entry ok'}else{Write-Error 'layout entry missing'}"
```
...
Run:
```
pwsh -NoProfile -Command "Test-Path 'wpf-net472-airgap-dev-pack/skills/collecting-wpf-net472-airgap-dev-pack-feedback/SKILL.md'; Test-Path 'FeedbackDocs/2026-05-18-customcontrol-authoring-upgrade-design.md'; (Get-Content 'wpf-net472-airgap-dev-pack/.claude-plugin/plugin.json' -Raw | ConvertFrom-Json).version; git status --porcelain"
```
...
Run:
```
pwsh -NoProfile -Command "Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-04-03-cmds-pattern-refactoring-design.md'; Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/plans/2026-04-03-cmds-pattern-refactoring.md'; Test-Path 'wpf-net472-airgap-dev-pack/docs/superpowers/specs/2026-05-18-collecting-wpf-devpack-feedback-design.md'"
```
